...
public class AddSessionServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            boolean status = ssnDAO.insertNewSession(ssn);

            if (status) {
                request.setAttribute("message", "New session successfully added.");
            } else {
                request.setAttribute("message", "Fail to add new session.");
            }
